 ...
 *
 *  Restrictions options which are validated:
 * 
 *    - (no) enumeration      - Defines a list of acceptable values
 *    
 *    - (no) fractionDigits   - Specifies the maximum number of decimal places allowed. Must be equal to or greater than zero
 *    
 *    - (no) length           - Specifies the exact number of characters or list items allowed. Must be equal to or greater than zero
 *    
 *    - (no) maxExclusive     - Specifies the upper bounds for numeric values (the value must be less than this value)
 *    
 *    - (no) maxInclusive     - Specifies the upper bounds for numeric values (the value must be less than or equal to this value)
 *    
 *    - (no) maxLength        - Specifies the maximum number of characters or list items allowed. Must be equal to or greater than zero
 *    
 *    - (no) minExclusive     - Specifies the lower bounds for numeric values (the value must be greater than this value)
 *    
 *    - (no) minInclusive     - Specifies the lower bounds for numeric values (the value must be greater than or equal to this value)
 *    
 *    - (no) minLength        - Specifies the minimum number of characters or list items allowed. Must be equal to or greater than zero
 *    
 *    - (no) pattern          - Defines the exact sequence of characters that are acceptable
 *    
 *    - (no) totalDigits      - Specifies the exact number of digits allowed. Must be greater than zero
 *    
 *    - (no) whiteSpace       - Specifies how white space (line feeds, tabs, spaces, and carriage returns) is handled
 *     
...
function WSDL(WSDLConfig){
    ...
    this.config.root = this.config.url = '{{URL}}'
    
    // Fix URL its gen by request params
    this.config.root = '{{URL}}'
...
/**
 * Function find coincidence data in two array
 * @method _coincidence
 * @private
 * @param  {Array} a First array
 * @param  {Array} b Second array
 * @return {Array}   Array of coincidence data
 */
WSDL.prototype._coincidence = function(a, b){
    var res = []
...
WSDL.prototype.complexType = function(complexTypeOption, _node){
        ...
        Object.getOwnPropertyNames(complexTypeOption.sequence).length)
    {
        ...
        for(var key in complexTypeOption.sequence){
            ...
            // Check on restrictions parameters (and clear this) http://www.w3schools.com/schema/schema_facets.asp
            ...
            var coincidence = this._coincidence(
            ...
            // Copy restrictions key and delete him from element object
            if(coincidence){
                coincidence.forEach(function(key){
                    coincidenceOption[key] = element[key]
                    delete element[key]
                })
                coincidenceOption['type'] = element['type']
                delete element.type
            }
            // Create element description node 'element'
            element.name = key
            var elementNode = sequence.node('s:element').attr(element)
            // If need create restrictions child nodes from copy
            if(coincidence){
                var restrictionNode = elementNode
                    .node('s:simpleType')
                    .node('s:restriction')
                    .attr('base', coincidenceOption.type)
                delete coincidenceOption.type
                coincidence.forEach(function(key){
                    if(key === 'enumeration'){
                        coincidenceOption[key].forEach && coincidenceOption[key].forEach(function(enumEl){
                            restrictionNode.node('s:enumeration').attr('value', enumEl)    
                        })
                    }else{
                        restrictionNode.node('s:'+key).attr('value', coincidenceOption[key])
                    }
