 ...
 * @return {Core.WSDLService}           		return WSDL object
 */
		...
		if(typeof req.query.wsdl !== 'undefined'){
			...
			res.end(
				me.wsdl
					// render XML
					.toString(1)
					// Replace template to request host + URL
 ...
 * @private
 */
WSDLService.prototype._executeBySOAP = function(req, res, requestURL){
 ...
 * @private
 */
WSDLService.prototype._executeByGET = function(req, res, requestURL){
	var requestObj = req.query
	var actionName = URL.parse(req.url).pathname.match(/\/(\w+)\/?$/)//[1]
	if(actionName && this.wsdl.ports[actionName[1]]){
		var me = this
 ...
 * @private
 */
 WSDLService.prototype._executeByPOST = function(req, res, requestURL){
...
// + '  <soap:Body>'
// + '    <tns:loginRequest>'
// + '      <tns:userLogin>lenfer</tns:userLogin>'
// + '      <tns:userPass>lenfer</tns:userPass>'
// + '      <tns:userType>53</tns:userType>'
// + '      <tns:applicationId>3</tns:applicationId>'
// + '      <tns:applicationKey>F0D74CBF46852E28C187151247DAFC374B9DBB35</tns:applicationKey>'
// + '    </tns:loginRequest>'
// + '  </soap:Body>'
// + '</soap:Envelope>'
